...
// Add this static variable at the top of the file, outside any functions (before QgisMobileapp constructor)
...
{
  // Check if this is a special reload with a flag to skip hardcoded layers
  bool skipHardcodedLayers = false;
  QString cleanPath = path;
  if (path.contains("#nohardcoded")) {
    skipHardcodedLayers = true;
    // Remove the flag from the path
    cleanPath = path;
    cleanPath.replace("#nohardcoded", "");
    QgsMessageLog::logMessage(
      QStringLiteral("Special project load detected with #nohardcoded flag - will skip adding hardcoded layers"),
  ...
  if ( !fi.exists() )
  {
    QgsMessageLog::logMessage( tr( "Can't load project, file \"%1\" does not exist" ).arg( cleanPath ), QStringLiteral( "SIGPACGO" ), Qgis::Warning );
    return false;
  }
  ...
  if ( SUPPORTED_PROJECT_EXTENSIONS.contains( suffix ) || SUPPORTED_VECTOR_EXTENSIONS.contains( suffix ) || SUPPORTED_RASTER_EXTENSIONS.contains( suffix ) )
  {
    ...
    mProjectFilePath = cleanPath;
    mProjectFileName = !name.isEmpty() ? name : fi.completeBaseName();
    
    // Store the skipHardcodedLayers flag for when readProjectFile is called
    mSkipHardcodedLayers = skipHardcodedLayers;
...
void QgisMobileapp::reloadProjectFile()
{
  ...
  {
    QgsMessageLog::logMessage( 
      tr( "No project file currently opened" ), 
      QStringLiteral( "SIGPACGO" ), 
      Qgis::Warning 
    );
    return;
  ...
  QgsMessageLog::logMessage(
    QStringLiteral("Reloading project file: %1")
      .arg(mProjectFilePath),
    QStringLiteral("SIGPACGO"),
    Qgis::Info
  );

  // Save the current layer visibility before reloading
  saveLayerVisibilityState();
  
  // Store the project path locally to ensure it's available during reload
  // Add the #nohardcoded flag to prevent hardcoded layers from being recreated during reload
  // This prevents duplication when a layer has been deleted
  const QString projectPath = mProjectFilePath + "#nohardcoded";
  const QString projectName = mProjectFileName;
  
  QgsMessageLog::logMessage(
    QStringLiteral("Adding #nohardcoded flag for reload to prevent layer duplication"),
    QStringLiteral("SIGPACGO"),
    Qgis::Info
  );
  
  emit loadProjectTriggered( projectPath, projectName );
}

void QgisMobileapp::readProjectFile()
{
  QgsMessageLog::logMessage(
    QStringLiteral("Starting readProjectFile for: %1")
      .arg(mProjectFilePath),
    QStringLiteral("SIGPACGO"),
    Qgis::Info
  );

  // Log if we are skipping hardcoded layers due to the flag being set in loadProjectFile
  if (mSkipHardcodedLayers) {
    QgsMessageLog::logMessage(
      QStringLiteral("Special project reload detected with skipHardcodedLayers flag - will skip adding hardcoded layers"),
      QStringLiteral("SIGPACGO"),
      Qgis::Info
    );
  }
  ...
  if ( projectLoaded )
  {
    ...
    QgsMessageLog::logMessage(tr("Failed to load project file: %1").arg(mProjectFilePath), 
  ...
  // Only create Google Satellite if we're not skipping hardcoded layers
  QgsRasterLayer *satelliteLayer = nullptr;
  if (!mSkipHardcodedLayers) {
    satelliteLayer = new QgsRasterLayer( QStringLiteral( "crs=EPSG:3857&format&type=xyz&url=http://www.google.cn/maps/vt?lyrs=s@189%26gl=cn%26x%3D{x}%26y%3D{y}%26z%3D{z}&zmax=21&zmin=0" ), QStringLiteral( "Google Satellite" ), QLatin1String( "wms" ) );
  } else {
    QgsMessageLog::logMessage(
      QStringLiteral("Skipping creation of Google Satellite layer due to skipHardcodedLayers flag"),
      QStringLiteral("SIGPACGO"),
      Qgis::Info
    );
  }
  ...
  QgsRasterLayer *sigpacLayer = nullptr;
  if (!mSkipHardcodedLayers) {
    sigpacLayer = new QgsRasterLayer(
    ...
    QgsMessageLog::logMessage(
      QStringLiteral("Skipping creation of Recintos SIGPAC FEGA layer due to skipHardcodedLayers flag"),
      QStringLiteral("SIGPACGO"),
      Qgis::Info
    );
  }
  ...
  QgsVectorTileLayer *teselaRecintosLayer = nullptr;
  if (!mSkipHardcodedLayers) {
    teselaRecintosLayer = new QgsVectorTileLayer(
    ...
    QgsMessageLog::logMessage(
      QStringLiteral("Skipping creation of TeselaRECINTOS FEGA layer due to skipHardcodedLayers flag"),
      QStringLiteral("SIGPACGO"),
      Qgis::Info
    );
  }
  ...
  // Only add TeselaRECINTOS if it exists
  if (teselaRecintosLayer) {
  ...
  // Only add SIGPAC layer if it exists
  if (sigpacLayer) {
  ...
  // Set TeselaRecintos FEGA opacity to 50% with safety check
  if (teselaRecintosLayer)
  {
    teselaRecintosLayer->setOpacity(0.5);
    QgsMessageLog::logMessage(
      QStringLiteral("Set TeselaRecintos FEGA opacity to 50%"),
      QStringLiteral("SIGPACGO"),
      Qgis::Info
    );
  }
  ...
  {
    QgsMessageLog::logMessage(
      QStringLiteral("TeselaRecintos FEGA layer not found or invalid when setting opacity"),
      QStringLiteral("SIGPACGO"),
      Qgis::Warning
    );
  }
  ...
  {
    QgsMessageLog::logMessage(
      QStringLiteral("Services GIS España group not found when setting visibility"),
      QStringLiteral("SIGPACGO"),
      Qgis::Warning
    );
  }
  
  if (spainGisBaseLayersGroup)
  {
  ...
  {
    QgsMessageLog::logMessage(
      QStringLiteral("Base Layers GIS España group not found when setting visibility"),
      QStringLiteral("SIGPACGO"),
      Qgis::Warning
    );
  }
  
  if (spainGisVectorTilesGroup)
  {
  ...
  {
    QgsMessageLog::logMessage(
      QStringLiteral("Vector Tiles GIS España group not found when setting visibility"),
      QStringLiteral("SIGPACGO"),
      Qgis::Warning
    );
  }
  ...
  {
    QgsMessageLog::logMessage(
      QStringLiteral("Utilidades group not found when setting visibility"),
      QStringLiteral("SIGPACGO"),
      Qgis::Warning
    );
  }
  ...
  {
    QgsMessageLog::logMessage(
      QStringLiteral("Basemaps group not found when setting visibility"),
      QStringLiteral("SIGPACGO"),
      Qgis::Warning
    );
  }
      ...
      for (QgsLayerTreeLayer* layer : sentinelGroup->findLayers())
      {
        ...
        if (!layerName.isEmpty() && enabledLayers.contains(layerName)) {
          ...
          if (style == "ON" || (layerName == "CUSTOM" && style == "DEFAULT")) {
            layer->setItemVisibilityChecked(true); // Set to true as this should be visible by default
          } else {
            // Only set to false if style is "OFF"
            layer->setItemVisibilityChecked(false);
          }
        } else {
          // Keep layers not in enabledLayers hidden
          layer->setItemVisibilityChecked(false);
        }
      }
    }
  ...
  // Move this connection setup earlier
  ...
  // Now restore layer visibility states for custom layers AFTER all other setup is done
  // The mSkipHardcodedLayers flag is now a member variable and used internally
  restoreLayerVisibilityState();
  
  // Only emit the loadProjectEnded signal at the very end
  // This ensures all our visibility states are set before UI updates
...
// Add this function to save layer visibility before reload
void QgisMobileapp::saveLayerVisibilityState()
{
  // First check if project path is valid
  if (mProjectFilePath.isEmpty())
  {
    QgsMessageLog::logMessage(
      QStringLiteral("Cannot save layer visibility - project file path is empty"),
      QStringLiteral("SIGPACGO"),
      Qgis::Warning
    );
    return;
  ...
  {
    QgsMessageLog::logMessage(
      QStringLiteral("Cannot save layer visibility - project layer tree root is null"),
      QStringLiteral("SIGPACGO"),
      Qgis::Warning
    );
    return;
  ...
  // Get list of hardcoded layers that we shouldn't save (they have fixed visibility)
  QStringList hardcodedLayerNames;
  hardcodedLayerNames << "Google Satellite" << "Recintos SIGPAC FEGA" << "TeselaRECINTOS FEGA";
  
  // Get list of Sentinel layers that also have their own rules
  QStringList sentinelLayerPrefixes;
  sentinelLayerPrefixes << "Sentinel ";

  QgsMessageLog::logMessage(
    QStringLiteral("Saving layer visibility for project: %1")
      .arg(mProjectFilePath),
    QStringLiteral("SIGPACGO"),
    Qgis::Info
  );
  ...
  mSavedLayerVisibility.clear();
  
  // Save state of all non-hardcoded layers
  int savedCount = 0;
  int skippedCount = 0;
  
  const QList<QgsLayerTreeLayer*> allLayers = mProject->layerTreeRoot()->findLayers();
  for (QgsLayerTreeLayer* layer : allLayers)
  {
    if (!layer || !layer->layer())
      continue;
      
    const QString layerName = layer->name();
    const QString layerId = layer->layer()->id();
    
    // Check if this is a hardcoded layer (which we won't save)
    bool isHardcodedLayer = false;
    for (const QString& hardcodedName : hardcodedLayerNames)
    {
      if (layerName.contains(hardcodedName))
      {
        isHardcodedLayer = true;
        break;
      }
    }
    
    // Check if this is a Sentinel layer (which follows its own rules)
    for (const QString& prefix : sentinelLayerPrefixes)
    {
    ...
    {
      QgsMessageLog::logMessage(
        QStringLiteral("Skipping hardcoded layer: %1 (ID: %2)")
          .arg(layerName)
          .arg(layerId),
        QStringLiteral("SIGPACGO"),
        Qgis::Info
    ...
    {
      // Save visibility state for non-hardcoded layer
      const bool isVisible = layer->itemVisibilityChecked();
      mSavedLayerVisibility[layerId] = isVisible;
      
      QgsMessageLog::logMessage(
        QStringLiteral("Saved custom layer: %1 (ID: %2) = %3")
          .arg(layerName)
          .arg(layerId)
          .arg(isVisible ? "visible" : "hidden"),
        QStringLiteral("SIGPACGO"),
        Qgis::Info
      );
      savedCount++;
  ...
  QgsMessageLog::logMessage(
    QStringLiteral("Layer visibility saving complete: %1 custom layers saved, %2 hardcoded layers skipped")
      .arg(savedCount)
      .arg(skippedCount),
    QStringLiteral("SIGPACGO"),
    Qgis::Info
...
// Add this function to restore layer visibility after reload
void QgisMobileapp::restoreLayerVisibilityState()
{
  // Enhanced safety checks - make sure we have everything we need
  if (mProjectFilePath.isEmpty())
  {
    QgsMessageLog::logMessage(
      QStringLiteral("Cannot restore layer visibility - project file path is empty"),
      QStringLiteral("SIGPACGO"),
      Qgis::Warning
    );
    return;
  ...
  {
    QgsMessageLog::logMessage(
      QStringLiteral("Cannot restore layer visibility - project is null"),
      QStringLiteral("SIGPACGO"),
      Qgis::Warning
    );
    return;
  ...
  QgsMessageLog::logMessage(
    QStringLiteral("Restoring layer visibility for project: %1%2")
      .arg(mProjectFilePath)
      .arg(mSkipHardcodedLayers ? " (skipping hardcoded layers)" : ""),
    QStringLiteral("SIGPACGO"),
    Qgis::Info
  );

  if (!mProject->layerTreeRoot())
  {
    QgsMessageLog::logMessage(
      QStringLiteral("Cannot restore layer visibility - layer tree root is null"),
      QStringLiteral("SIGPACGO"),
      Qgis::Warning
    );
    return;
  ...
  // Get all layers from the current project's layer tree
  QList<QgsLayerTreeLayer*> allLayers = mProject->layerTreeRoot()->findLayers();
  
  // Master map to track all layers by their name to detect duplicates
  QMap<QString, QList<QgsLayerTreeLayer*>> allLayersByName;
  
  // Track layers to be removed for deferred removal (to avoid iterator invalidation)
  QList<QPair<QgsLayerTreeGroup*, QgsLayerTreeLayer*>> layersToRemove;
  QList<QString> layerIdsToRemove;
  
  // Our hardcoded layers by type - these will always be visible
  QStringList alwaysVisibleLayers;
  alwaysVisibleLayers << QStringLiteral("Google Satellite") 
                     << QStringLiteral("Recintos SIGPAC FEGA") 
                     << QStringLiteral("TeselaRECINTOS FEGA");
  
  // Group names that might contain duplicate layers
  QStringList hardcodedGroups;
  hardcodedGroups << QStringLiteral("Capas Base GIS España") 
                 << QStringLiteral("Utilidades")
                 << QStringLiteral("Imágenes Sentinel")
                 << QStringLiteral("Servicios GIS España");  // Add this group
                 
  // Specific groups to check and remove if empty
  QStringList groupsToCheckForEmpty;
  groupsToCheckForEmpty << QStringLiteral("Servicios GIS España")
                       << QStringLiteral("Capas Base GIS España") 
                       << QStringLiteral("Utilidades");
  
  // First, categorize all layers by name
  for (QgsLayerTreeLayer* layer : allLayers)
  {
    // Skip null layers or layers without valid QgsMapLayer
    if (!layer || !layer->layer())
      continue;
      
    QString layerName = layer->name();
    
    // Add the layer to our tracking map
    if (!allLayersByName.contains(layerName))
    {
      allLayersByName[layerName] = QList<QgsLayerTreeLayer*>();
    }
    
    // Check if it's already in the list to avoid duplicates
    bool alreadyInList = false;
    for (QgsLayerTreeLayer* existingLayer : allLayersByName[layerName])
    {
    ...
    if (!alreadyInList)
    {
      allLayersByName[layerName].append(layer);
      
      // Log special hardcoded layer types
      if (alwaysVisibleLayers.contains(layerName))
      {
        QgsMessageLog::logMessage(
          QStringLiteral("Found hardcoded layer: %1 (ID: %2)")
            .arg(layerName)
            .arg(layer->layer()->id()),
          QStringLiteral("SIGPACGO"),
          Qgis::Info
  ...
  // Additional pass to classify layers by group and source URL
  QMap<QString, QMap<QString, QList<QgsLayerTreeLayer*>>> layersBySourceInGroup;
  
  // Find all layers within hardcoded groups and organize them by source URL
  for (QgsLayerTreeLayer* layer : allLayers)
  {
    if (!layer || !layer->layer())
      continue;
      
    // Get the parent group of this layer
    QgsLayerTreeGroup* parentGroup = nullptr;
    QgsLayerTreeNode* parent = layer->parent();
    while (parent)
    {
      parentGroup = qobject_cast<QgsLayerTreeGroup*>(parent);
      if (parentGroup && hardcodedGroups.contains(parentGroup->name()))
      {
        // Found one of our hardcoded groups as a parent
        const QString groupName = parentGroup->name();
        
        // Get the layer source URL
        QString sourceUrl;
        if (QgsRasterLayer* rasterLayer = qobject_cast<QgsRasterLayer*>(layer->layer()))
        {
          sourceUrl = rasterLayer->source();
        }
        else if (QgsVectorLayer* vectorLayer = qobject_cast<QgsVectorLayer*>(layer->layer()))
        {
          sourceUrl = vectorLayer->source();
        }
        
        if (!sourceUrl.isEmpty())
        {
          // Initialize the group map if not existing
          if (!layersBySourceInGroup.contains(groupName))
          {
            layersBySourceInGroup[groupName] = QMap<QString, QList<QgsLayerTreeLayer*>>();
          }
          
          // Initialize the source list if not existing
          if (!layersBySourceInGroup[groupName].contains(sourceUrl))
          {
            layersBySourceInGroup[groupName][sourceUrl] = QList<QgsLayerTreeLayer*>();
          }
          
          // First check if the layer is already in the list to avoid duplicates
          bool alreadyInList = false;
          for (QgsLayerTreeLayer* existingLayer : layersBySourceInGroup[groupName][sourceUrl]) {
            if (existingLayer == layer) {
              alreadyInList = true;
              break;
            }
          ...
          if (!alreadyInList) {
            layersBySourceInGroup[groupName][sourceUrl].append(layer);
            
            QgsMessageLog::logMessage(
              QStringLiteral("Found layer in hardcoded group: %1 (Group: %2, URL: %3)")
                .arg(layer->name())
                .arg(groupName)
                .arg(sourceUrl),
              QStringLiteral("SIGPACGO"),
              Qgis::Info  // Changed from Debug to Info since Debug doesn't exist in this version
            );
          } else {
            QgsMessageLog::logMessage(
              QStringLiteral("Skipping duplicate entry for layer: %1 (already in list)")
                .arg(layer->name()),
              QStringLiteral("SIGPACGO"),
              Qgis::Info
            );
        ...
        break; // No need to check higher-level parents
      }
  ...
  // Count of layers handled
  int hardcodedCount = 0;
  int duplicatesRemoved = 0;
  int restoredCount = 0;
  // Track total empty groups removed
  int totalEmptyGroupsRemoved = 0;
  
  // Skip this step if skipHardcodedLayers is true
  if (!mSkipHardcodedLayers)
  {
    // First handle the special always-visible hardcoded layers
    for (const QString& hardcodedType : alwaysVisibleLayers)
    {
      if (!allLayersByName.contains(hardcodedType))
        continue;
        
      const QList<QgsLayerTreeLayer*>& layers = allLayersByName[hardcodedType];
      
      // If we have duplicates of this hardcoded layer type
      if (layers.size() > 1)
      {
        QgsMessageLog::logMessage(
          QStringLiteral("Found %1 duplicate layers for type: %2 - will keep only the first one")
            .arg(layers.size())
            .arg(hardcodedType),
          QStringLiteral("SIGPACGO"),
          Qgis::Warning
        ...
        QgsLayerTreeLayer* layerToKeep = layers.first();
        
        // Set the one we're keeping to visible
        if (layerToKeep && layerToKeep->layer())
        {
          layerToKeep->setItemVisibilityChecked(true);
          
          QgsMessageLog::logMessage(
            QStringLiteral("Keeping hardcoded layer: %1 (ID: %2)")
              .arg(layerToKeep->name())
              .arg(layerToKeep->layer()->id()),
            QStringLiteral("SIGPACGO"),
            Qgis::Info
          );
          hardcodedCount++;
          
          // Mark all duplicates for deferred removal
          for (int i = 1; i < layers.size(); i++)
          {
            QgsLayerTreeLayer* layerToRemove = layers[i];
            if (layerToRemove && layerToRemove->layer())
            {
              QgsMessageLog::logMessage(
                QStringLiteral("Removing duplicate hardcoded layer: %1 (ID: %2)")
                  .arg(layerToRemove->name())
                  .arg(layerToRemove->layer()->id()),
                QStringLiteral("SIGPACGO"),
                Qgis::Info
              );
              
              // Get the ID before removing
              QString layerId = layerToRemove->layerId();
              
              // Add safety checks to prevent crashes
              if (layerId.isEmpty()) {
                QgsMessageLog::logMessage(
                  QStringLiteral("Cannot remove hardcoded layer with empty ID"),
                  QStringLiteral("SIGPACGO"),
                  Qgis::Warning
                );
                continue;
              ...
              // Check if the layer is still in the project
              if (!mProject->mapLayer(layerId)) {
                QgsMessageLog::logMessage(
                  QStringLiteral("Hardcoded layer %1 not found in project, skipping removal")
                    .arg(layerId),
                  QStringLiteral("SIGPACGO"),
                  Qgis::Warning
                );
              ...
              // Instead of removing immediately, add to list for deferred removal
              QgsLayerTreeGroup* parent = qobject_cast<QgsLayerTreeGroup*>(layerToRemove->parent());
              if (parent)
              {
                layersToRemove.append(qMakePair(parent, layerToRemove));
      ...
      {
        // Just one layer of this type - set it to visible
        QgsLayerTreeLayer* layer = layers.first();
        if (layer && layer->layer())
        {
          // Always set these layers to visible
          layer->setItemVisibilityChecked(true);
          
          QgsMessageLog::logMessage(
            QStringLiteral("Set hardcoded layer to visible: %1 (ID: %2)")
              .arg(layer->name())
              .arg(layer->layer()->id()),
            QStringLiteral("SIGPACGO"),
            Qgis::Info
    ...
    // Handle all other layers by name
    for (const QString& layerName : allLayersByName.keys())
    {
      // Skip the special hardcoded layers we already processed
      if (alwaysVisibleLayers.contains(layerName))
        continue;
      
      const QList<QgsLayerTreeLayer*>& layers = allLayersByName[layerName];
      ...
      QgsMessageLog::logMessage(
        QStringLiteral("Found %1 duplicate layers with name: %2 - will keep only the first one")
          .arg(layers.size())
          .arg(layerName),
        QStringLiteral("SIGPACGO"),
        Qgis::Warning
      ...
      // Keep the first layer and mark the rest for removal
      QgsLayerTreeLayer* layerToKeep = layers.first();
      
      // Make sure the layer to keep is valid
      if (!layerToKeep || !layerToKeep->layer()) {
        QgsMessageLog::logMessage(
          QStringLiteral("First layer in duplicates list is invalid, skipping layer: %1")
            .arg(layerName),
          QStringLiteral("SIGPACGO"),
          Qgis::Warning
        );
      ...
      // Keep the visibility state of the first layer
      bool isVisible = layerToKeep->itemVisibilityChecked();
      
      QgsMessageLog::logMessage(
        QStringLiteral("Keeping layer: %1 (visibility: %2)")
          .arg(layerToKeep->name())
          .arg(isVisible ? "visible" : "hidden"),
        QStringLiteral("SIGPACGO"),
        Qgis::Info
      ...
      for (int i = 1; i < layers.size(); i++)
      {
        ...
        {
          QgsMessageLog::logMessage(
            QStringLiteral("Removing duplicate layer: %1 (ID: %2)")
              .arg(layerToRemove->name())
              .arg(layerToRemove->layer()->id()),
            QStringLiteral("SIGPACGO"),
            Qgis::Info
          ...
          if (layerId.isEmpty()) {
            QgsMessageLog::logMessage(
              QStringLiteral("Cannot remove layer with empty ID"),
              QStringLiteral("SIGPACGO"),
              Qgis::Warning
            );
            continue;
          ...
          if (!mProject->mapLayer(layerId)) {
            QgsMessageLog::logMessage(
              QStringLiteral("Layer %1 not found in project, skipping removal")
                .arg(layerId),
              QStringLiteral("SIGPACGO"),
              Qgis::Warning
            );
    ...
    // As a backup, also process layers by source URL within groups
    // This might be unnecessary with the above approach but keeping it for safety
    for (const QString& groupName : layersBySourceInGroup.keys())
    {
      QMap<QString, QList<QgsLayerTreeLayer*>> sourceMap = layersBySourceInGroup[groupName];
      
      QgsMessageLog::logMessage(
        QStringLiteral("Processing group: %1 with %2 different layer sources")
          .arg(groupName)
          .arg(sourceMap.size()),
        QStringLiteral("SIGPACGO"),
        Qgis::Info
      ...
      // For each source URL in this group
      for (const QString& sourceUrl : sourceMap.keys())
      {
        QList<QgsLayerTreeLayer*> layersWithSameSource = sourceMap[sourceUrl];
        
        // Skip sources with only one layer (no duplicates)
        if (layersWithSameSource.size() <= 1)
          continue;
        
        // Check if we already added these layers to the removal list by name
        bool alreadyProcessed = true;
        for (int i = 1; i < layersWithSameSource.size(); i++)
        {
          QgsLayerTreeLayer* layer = layersWithSameSource[i];
          if (layer && layer->layer())
          {
            QString layerId = layer->layerId();
            if (!layerIdsToRemove.contains(layerId))
            {
        ...
        {
          QgsMessageLog::logMessage(
            QStringLiteral("Skipping already processed layers with source: %1")
              .arg(sourceUrl),
            QStringLiteral("SIGPACGO"),
            Qgis::Info
          );
        ...
        QgsMessageLog::logMessage(
          QStringLiteral("Found %1 duplicate layers with same source in group %2")
            .arg(layersWithSameSource.size())
            .arg(groupName),
          QStringLiteral("SIGPACGO"),
          Qgis::Warning
        );
        
        // Keep the first layer, remove the rest
        QgsLayerTreeLayer* layerToKeep = layersWithSameSource.first();
        
        // Make sure the layer to keep is valid
        if (!layerToKeep || !layerToKeep->layer()) {
          QgsMessageLog::logMessage(
            QStringLiteral("First layer in duplicates list is invalid, skipping source: %1")
              .arg(sourceUrl),
            QStringLiteral("SIGPACGO"),
            Qgis::Warning
          );
        ...
        for (int i = 1; i < layersWithSameSource.size(); i++)
        {
          QgsLayerTreeLayer* layerToRemove = layersWithSameSource[i];
          if (layerToRemove && layerToRemove->layer())
          {
            QgsMessageLog::logMessage(
              QStringLiteral("Removing duplicate layer: %1 from group %2")
                .arg(layerToRemove->name())
                .arg(groupName),
              QStringLiteral("SIGPACGO"),
              Qgis::Info
            ...
            // Check if we've already marked this layer for removal
            if (layerIdsToRemove.contains(layerId)) {
              QgsMessageLog::logMessage(
                QStringLiteral("Layer %1 already marked for removal")
                  .arg(layerId),
                QStringLiteral("SIGPACGO"),
                Qgis::Info
              );
    ...
    // Deferred removal to avoid iterator invalidation
    for (int i = 0; i < layersToRemove.size(); i++) {
      QgsLayerTreeGroup* parent = layersToRemove[i].first;
      QgsLayerTreeLayer* layerToRemove = layersToRemove[i].second;
      QString layerId = layerIdsToRemove[i];
      
      // Final safety check
      if (!parent || !layerToRemove || !layerToRemove->layer()) {
        QgsMessageLog::logMessage(
          QStringLiteral("Invalid layer or parent for deferred removal, skipping."),
          QStringLiteral("SIGPACGO"),
          Qgis::Warning
        );
        continue;
      ...
      try {
        parent->removeChildNode(layerToRemove);
        mProject->removeMapLayer(layerId);
        duplicatesRemoved++;
        
        QgsMessageLog::logMessage(
          QStringLiteral("Successfully removed duplicate layer with ID: %1")
            .arg(layerId),
          QStringLiteral("SIGPACGO"),
          Qgis::Info
        );
        ...
        QgsMessageLog::logMessage(
          QStringLiteral("Error removing layer %1: %2")
            .arg(layerId)
            .arg(e.what()),
          QStringLiteral("SIGPACGO"),
          Qgis::Critical
        ...
        QgsMessageLog::logMessage(
          QStringLiteral("Unknown error removing layer %1")
            .arg(layerId),
          QStringLiteral("SIGPACGO"),
          Qgis::Critical
        );
    ...
    // After removing all duplicate layers, cleanup empty groups
    QgsMessageLog::logMessage(
      QStringLiteral("Scanning for empty groups to remove"),
      QStringLiteral("SIGPACGO"),
      Qgis::Info
    );
    ...
    // Function to check if a group is truly empty (no layers, including in subgroups)
    std::function<bool(QgsLayerTreeGroup*)> isGroupEmpty = [](QgsLayerTreeGroup* group) -> bool {
      if (!group) return true;
      
      QList<QgsLayerTreeLayer*> layers = group->findLayers();
      return layers.isEmpty();
    };
    
    // Function to remove empty groups
    std::function<void(QgsLayerTreeGroup*)> removeEmptyGroup = [&](QgsLayerTreeGroup* group) {
      if (!group || group == root) return;
      
      QString groupName = group->name();
      QgsLayerTreeGroup* parent = qobject_cast<QgsLayerTreeGroup*>(group->parent());
      if (!parent) return;
      
      try {
        QgsMessageLog::logMessage(
          QStringLiteral("Removing empty group: %1")
            .arg(groupName),
          QStringLiteral("SIGPACGO"),
          Qgis::Info
        );
        ...
        totalEmptyGroupsRemoved++;
        
        QgsMessageLog::logMessage(
          QStringLiteral("Successfully removed empty group: %1")
            .arg(groupName),
          QStringLiteral("SIGPACGO"),
          Qgis::Info
        );
        ...
        QgsMessageLog::logMessage(
          QStringLiteral("Error removing empty group %1: %2")
            .arg(groupName)
            .arg(e.what()),
          QStringLiteral("SIGPACGO"),
          Qgis::Critical
    ...
    QList<QgsLayerTreeGroup*> allGroups = root->findGroups();
    
    // First, handle Servicios GIS España and its subgroups
    QgsLayerTreeGroup* serviciosGroup = nullptr;
    for (QgsLayerTreeGroup* group : allGroups) {
      if (group && group->name() == "Servicios GIS España") {
        serviciosGroup = group;
        break;
      }
    }
    ...
    if (serviciosGroup) {
      QgsMessageLog::logMessage(
        QStringLiteral("Found Servicios GIS España group for special handling"),
        QStringLiteral("SIGPACGO"),
        Qgis::Info
      );
      
      // First remove empty subgroups
      QList<QgsLayerTreeGroup*> subgroups = serviciosGroup->findGroups();
      for (QgsLayerTreeGroup* subgroup : subgroups) {
        if (isGroupEmpty(subgroup)) {
          removeEmptyGroup(subgroup);
        }
      }
      
      // Then check if main group is empty
      if (isGroupEmpty(serviciosGroup)) {
        removeEmptyGroup(serviciosGroup);
      }
    }
    
    // Handle other main groups
    QStringList groupsToCheck = {"Utilidades", "Mapas Base"};
    for (const QString& groupName : groupsToCheck) {
      for (QgsLayerTreeGroup* group : allGroups) {
        if (group && group->name() == groupName) {
          // Check subgroups first
          QList<QgsLayerTreeGroup*> subgroups = group->findGroups();
          for (QgsLayerTreeGroup* subgroup : subgroups) {
            if (isGroupEmpty(subgroup)) {
              removeEmptyGroup(subgroup);
            }
    ...
    // Final pass to remove any remaining empty groups
    allGroups = root->findGroups(); // Refresh the list
    for (QgsLayerTreeGroup* group : allGroups) {
      if (group && group != root && isGroupEmpty(group)) {
        removeEmptyGroup(group);
      }
    }
    
    QgsMessageLog::logMessage(
      QStringLiteral("Layer visibility restoration complete: %1 hardcoded layers set visible, %2 custom layers restored, %3 duplicate layers removed, %4 empty groups removed")
        .arg(hardcodedCount)
        .arg(restoredCount)
        .arg(duplicatesRemoved)
        .arg(totalEmptyGroupsRemoved),
      QStringLiteral("SIGPACGO"),
      Qgis::Info
    );
  }
  ...
  {
    QgsMessageLog::logMessage(
      QStringLiteral("Skipping visibility setting for hardcoded layers as requested"),
      QStringLiteral("SIGPACGO"),
      Qgis::Info
    );
  }
