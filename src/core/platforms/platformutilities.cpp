...
void PlatformUtilities::copySigpacBaseMap()
{
  ...
  qDebug() << "Using app data directory for SIGPAC_BASE:" << targetDir;
  ...
  // Copy the base map using the same approach as sample projects
  FileUtils::copyRecursively( systemSharedDataLocation() + QLatin1String( "/resources/sigpacgo_base" ), targetDir );
}
